// script.js — Lógica principal da aplicação Correição
// Depende de: config.js, Secoes.js, Supabase SDK, html2pdf.js
...
const sb = supabase.createClient(CONFIG.SUPABASE_URL, CONFIG.SUPABASE_ANON, {
  global: {
    headers: {
      'x-user-id': (function () {
        try {
          const a = JSON.parse(localStorage.getItem('msalAccount') || '{}');
...
function toggleMenu() {
  ...
  mb.style.display = mb.style.display === 'none' ? 'inline-block' : 'none';
}
...
function criarMenu() {
  ...
  let sepNovo = document.createElement('a');
  sepNovo.innerText = '➕ Nova correição';
  sepNovo.style.cssText = 'margin-top:16px;border-top:1px solid rgba(255,255,255,0.18);padding-top:16px;color:#4FB8EE;';
  sepNovo.onclick = () => { novaCorreicao(); toggleMenu(); };
  scroll.appendChild(sepNovo);

  let sepHistorico = document.createElement('a');
  sepHistorico.innerText = '📋 Histórico';
  sepHistorico.style.cssText = 'margin-top:8px;color:#4FB8EE;';
  sepHistorico.onclick = () => { abrirHistorico(); toggleMenu(); };
  scroll.appendChild(sepHistorico);

  let logoutLink = document.createElement('a');
  logoutLink.innerText = 'Sair';
  logoutLink.style.cssText = 'margin-top:8px;border-top:1px solid rgba(255,255,255,0.18);padding-top:16px;';
  logoutLink.onclick = logout;
  scroll.appendChild(logoutLink);
}
    ...
    if (sec.subtopicos) {
      ...
      function atualizarCampos() {
        ...
        sec.subtopicos.forEach(sub => {
          ...
          sec.campos.filter(c => c.id.startsWith(sub.prefixo + '.')).forEach(campo => render(camposWrap, campo));
        });
        carregar();
      }
      ...
      sec.subtopicos.forEach(sub => {
        ...
        chip.onclick = () => {
          if (ativos.has(sub.prefixo)) { ativos.delete(sub.prefixo); chip.classList.remove('chip-ativo'); }
          else { ativos.add(sub.prefixo); chip.classList.add('chip-ativo'); }
          atualizarCampos();
        };
...
function render(p, c) {
  ...
  wrapper.dataset.campoId = c.id;
  if (c.dependeDe) {
    wrapper.dataset.dependeDeId    = c.dependeDe.id;
    wrapper.dataset.dependeDeValor = c.dependeDe.valor;
  }
  ...
  if (c.tipo === 'textarea') { let t = document.createElement('textarea'); t.id = c.id; t.required = true; t.oninput = autoSalvar; p.appendChild(t); }
  if (c.tipo === 'date')     { let i = document.createElement('input'); i.type = 'date'; i.id = c.id; i.required = true; i.onchange = autoSalvar; p.appendChild(i); }
  if (c.tipo === 'time')     { let i = document.createElement('input'); i.type = 'time'; i.id = c.id; i.required = true; i.onchange = autoSalvar; p.appendChild(i); }
  if (c.tipo === 'number')   { let i = document.createElement('input'); i.type = 'number'; i.id = c.id; i.required = true; i.oninput = autoSalvar; p.appendChild(i); }
  if (c.tipo === 'text')     { let i = document.createElement('input'); i.type = 'text'; i.id = c.id; i.required = true; i.oninput = autoSalvar; p.appendChild(i); }
  if (c.tipo === 'radio') {
    ...
    c.opcoes.forEach(op => {
      ...
      r.type = 'radio'; r.name = c.id; r.value = op; r.required = true;
...
function avaliarCondicionais() {
  document.querySelectorAll('[data-depende-de-id]').forEach(wrapper => {
    let controlId     = wrapper.dataset.dependeDeId;
    let valorEsperado = wrapper.dataset.dependeDeValor;
    let controlEl     = document.querySelector(`input[name="${controlId}"]:checked`);
    let valorAtual    = controlEl ? controlEl.value : null;
    let obrigatorio   = (valorAtual === valorEsperado);
...
function btn(text, cls, fn) {
  ...
  b.innerText = text; b.className = cls; b.onclick = fn;
  return b;
}
...
function carregar() {
  let dadosSalvos = window._dadosCarregados || {};
  Object.keys(dadosSalvos).forEach(k => {
    let el = document.getElementById(k) || document.querySelector(`input[name="${k}"][value="${dadosSalvos[k]}"]`);
    if (!el) return;
    if (el.type === 'radio') {
      let r = document.querySelector(`input[name="${k}"][value="${dadosSalvos[k]}"]`);
      if (r) r.checked = true;
    } else {
...
/* ====  VALIDAÇÃO  ==== */
function limparInvalidos(base) { base.querySelectorAll('.invalid').forEach(el => el.classList.remove('invalid')); }
function encontrarInvalidos(base) {
  limparInvalidos(base);
  let invalids = []; let radiosProc = new Set();
  base.querySelectorAll('input,textarea').forEach(el => {
    ...
    if (el.required && (!el.value || !el.value.toString().trim())) { el.classList.add('invalid'); invalids.push(el); }
  });
  return invalids;
}
function focusFirstInvalid(invalids) {
  ...
  if (first.classList.contains('radio-group')) { let r = first.querySelector('input[type="radio"]'); if (r) r.focus(); }
  else { first.focus(); }
...
async function garantirRegistroAberto() {
  ...
  let { data, error } = await sb
    .from('correicoes').select('*')
    .eq('user_id', userId).eq('finalizado', false)
    .order('criado_em', { ascending: false }).limit(1);
  if (error) throw error;
  if (data && data.length > 0) return data[0];

  let { data: novo, error: errNovo } = await sb
    .from('correicoes')
    .insert({ user_id: userId, nome: getNomeUsuario(), dados: {}, secoes_ok: {}, finalizado: false })
    .select().single();
...
function autoSalvar() {
  ...
  _autoSalvarTimer = setTimeout(() => { persistirDados(); }, 1500);
}
...
async function persistirDados() {
  ...
  await sb.from('correicoes').update({ dados, atualizado_em: new Date().toISOString() }).eq('id', registroId);
}

/* ====  SALVAR SEÇÃO  ==== */
async function salvarSecao(i) {
  if (modoLeitura) { showToast('Este envio já foi finalizado e não pode ser alterado.', 'erro'); return; }

  let section  = document.querySelectorAll('.section')[i];
  let invalidos = encontrarInvalidos(section);
  if (invalidos.length) { mostrar(i); focusFirstInvalid(invalidos); showToast('Preencha todos os campos obrigatórios antes de salvar a seção.', 'erro'); return; }

  mostrarLoading('Salvando seção…');
  try {
    const dadosSecao  = coletar(i);
    const dadosMerged = Object.assign({}, window._dadosCarregados || {}, dadosSecao);
    window._dadosCarregados = dadosMerged;

    let { data: reg } = await sb.from('correicoes').select('secoes_ok').eq('id', registroId).single();
    let secoesOk = reg ? (reg.secoes_ok || {}) : {};
    secoesOk[i] = 'true';

    await sb.from('correicoes').update({ dados: dadosMerged, secoes_ok: secoesOk, atualizado_em: new Date().toISOString() }).eq('id', registroId);
    bloquearSecao(i);
...
async function enviarTudo() {
  if (modoLeitura) { showToast('Este envio já foi finalizado.', 'erro'); return; }
  ...
  try {
    ...
    await sb.from('correicoes').update({ dados: dadosCampos, finalizado: true, atualizado_em: dataEnvio }).eq('id', registroId);

    const payload = { registro_id: registroId, email, nome, data_envio: dataEnvio };
    const r = await fetch(CONFIG.PA_CRIAR, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
...
async function novaCorreicao() {
  if (registroId && !modoLeitura) {
    let { data: reg } = await sb.from('correicoes').select('secoes_ok').eq('id', registroId).single();
    let secoesOk = reg ? (reg.secoes_ok || {}) : {};
    let pendentes = form.secoes.length - Object.keys(secoesOk).length;
    if (pendentes > 0) {
      if (!confirm(`Você tem ${pendentes} seção(ões) não salva(s) no registro atual.\n\nDeseja realmente iniciar uma nova correição? O rascunho atual será mantido.`)) return;
    }
  }
  ...
  try {
    const userId = getEmailUsuario();
    let { data: novo, error } = await sb.from('correicoes')
      .insert({ user_id: userId, nome: getNomeUsuario(), dados: {}, secoes_ok: {}, finalizado: false })
      .select().single();
    if (error) throw error;
    ...
    document.querySelector('.modo-leitura-aviso') && document.querySelector('.modo-leitura-aviso').remove();
    avaliarCondicionais();
    abrirFormulario();
    mostrar(0);
...
async function abrirHistorico() {
  ...
  try {
    let { data, error } = await sb
      .from('correicoes').select('id, criado_em, atualizado_em, finalizado, secoes_ok')
      .eq('user_id', getEmailUsuario()).order('criado_em', { ascending: false });
    if (error) throw error;
    renderHistorico(data || [], hc);
  } catch (err) {
...
function renderHistorico(registros, container) {
  ...
  registros.forEach(reg => {
    ...
    card.style.cssText = `background:#fff;border-radius:10px;padding:18px 20px;margin-bottom:14px;
      box-shadow:0 2px 8px rgba(0,0,0,0.08);
      border-left:4px solid ${reg.finalizado ? '#27ae60' : '#1C799B'};
      display:flex;justify-content:space-between;align-items:flex-start;gap:18px;`;

    let dataStr = isNaN(new Date(reg.criado_em)) ? reg.criado_em : new Date(reg.criado_em).toLocaleString('pt-BR');
    let secoesOk = Object.keys(reg.secoes_ok || {}).length;
    let status   = reg.finalizado
    ...
    let btnAbrir   = card.querySelector('.btn-primary');
    let btnPDF     = card.querySelector('.btn-gray');
    let btnExcluir = card.querySelector('.btn-danger');
    ...
    if (reg.finalizado) {
      ...
      btnExcluir.onclick = () => excluirRegistro(reg.id);
    }
...
async function carregarRegistro(id) {
  ...
  try {
    let { data: reg, error } = await sb.from('correicoes').select('*').eq('id', id).single();
    if (error) throw error;
    ...
    carregar();
    let secoesOk = reg.secoes_ok || {};
    Object.keys(secoesOk).forEach(i => { bloquearSecao(Number(i)); atualizarMenuBadge(Number(i), true); });
...
async function excluirRegistro(id) {
  ...
  try {
    const { data: reg, error: erroBusca } = await sb.from('correicoes').select('id, finalizado').eq('id', id).single();
    if (erroBusca) throw erroBusca;
    if (reg.finalizado) { showToast('Registros finalizados não podem ser excluídos.', 'erro'); return; }
    const { error: erroDelete } = await sb.from('correicoes').delete().eq('id', id);
...
<div class="pdf-page" id="pdf-conteudo">
  <div class="pdf-banner"><img src="logo.png" /></div>
  ...
  <div class="pdf-cabecalho">
    ...
    form.secoes.forEach(sec => {
      const renderCampo = (campo, container) => {
        ...
        const div  = document.createElement('div'); div.className = 'pdf-campo';
        const perg = document.createElement('div'); perg.className = 'pdf-pergunta'; perg.textContent = campo.pergunta;
        const val  = document.createElement('div'); val.className  = 'pdf-valor';    val.textContent  = valor;
        div.appendChild(perg); div.appendChild(val);
        container.appendChild(div);
        return true;
      };

      // Coleta todos os elementos desta seção em um fragmento temporário
      ...
      // Só adiciona o título da seção se houver campos respondidos
      if (secTemConteudo) {
    ...
    const opt = {
      margin: 10, filename: `correicao-${data.id}.pdf`,
...
function atualizarMenuBadge(i, enviada = true) {
  let link  = document.getElementById(`menu-sec-${i}`);
  if (!link) return;
  let badge = link.querySelector('.badge-enviada');
  if (enviada && !badge) { let b = document.createElement('span'); b.className = 'badge-enviada'; b.textContent = '✓'; link.appendChild(b); }
  else if (!enviada && badge) { badge.remove(); }
}
...
async function init() {
  ...
  try {
    ...
    let reg = await garantirRegistroAberto();
    registroId  = reg.id;
    modoLeitura = reg.finalizado;
    window._dadosCarregados = reg.dados || {};
